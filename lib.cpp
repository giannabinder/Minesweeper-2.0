...
bool game() {
  ...
  while (currAction != 'Q') {
    ...
    if (isGameWon(gameBoard, xdim, ydim)) {
      std::cout << "You have revealed all the fields without exploding a bomb!"
                << std::endl;
      std::cout << "YOU WON!!!" << std::endl;
      for (std::size_t reveal_row{0}; reveal_row < ydim; reveal_row++) {
        for (std::size_t reveal_col{0}; reveal_col < xdim; reveal_col++) {
          gameBoard[reveal_row * xdim + reveal_col] =
              (gameBoard[reveal_row * xdim + reveal_col] & valueMask());
        }
      }
      printBoard(gameBoard, xdim, ydim);
...
void startGame(char *&board, std::size_t &xdim, std::size_t &ydim,
  ...
  std::cout << "Welcome to BrainSweeper!" << std::endl;
  do {
  ...
  while (numgeese > xdim * ydim) {
    std::cout << "That's too many bombs!" << std::endl;
    std::cout << "Please enter the number of bombs: ";
    std::cin >> numgeese;
  }
...
void actionShow(char *&board, std::size_t &xdim, std::size_t &ydim,
  ...
  if (reveal_x >= xdim || reveal_y >= ydim) {
    ...
    std::cout << "BOOM! Your game has ended." << std::endl;
...
std::size_t readSizeT() { return 0; }

std::size_t xdim_max() { return 60; }

std::size_t ydim_max() { return 20; }
